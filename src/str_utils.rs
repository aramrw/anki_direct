...
///
/// ```no_run
/// # use clippy_utils::str_utils::{_camel_case_until, StrIndex};
/// assert_eq!(_camel_case_until("AbcDef"), StrIndex::new(6, 6));
/// assert_eq!(_camel_case_until("ABCD"), StrIndex::new(0, 0));
/// assert_eq!(_camel_case_until("AbcDD"), StrIndex::new(3, 3));
/// assert_eq!(_camel_case_until("Abc\u{f6}\u{f6}DD"), StrIndex::new(5, 7));
/// ```
#[must_use]
pub fn _camel_case_until(s: &str) -> StrIndex {
...
impl StrCount {
    pub fn _new(char_count: usize, byte_count: usize) -> Self {
        Self {
            char_count,
            byte_count,
        }
...
/// ```
#[must_use]
pub fn _count_match_start(str1: &str, str2: &str) -> StrCount {
    // (char_index, char1)
    ...
    iter1
        ...
        .map_or_else(
            ...
            |((char_index, _), (byte_index, character))| {
                StrCount::_new(char_index + 1, byte_index + character.len_utf8())
            },
        )
}
...
/// ```
#[must_use]
pub fn _count_match_end(str1: &str, str2: &str) -> StrCount {
    let char_count = str1.chars().count();
    if char_count == 0 {
    ...
    iter1
        ...
        .map_or_else(StrCount::default, |((char_index, _), (byte_index, _))| {
            StrCount::_new(char_count - char_index, byte_count - byte_index)
        })
}
...
/// ```
pub fn _to_snake_case(name: &str) -> String {
...
/// ```
pub fn _to_camel_case(item_name: &str) -> String {
    let mut s = String::new();
    let mut up = true;
...
mod test {
    ...
    #[test]
    fn _camel_case_until_full() {
        assert_eq!(_camel_case_until("AbcDef"), StrIndex::new(6, 6));
        assert_eq!(_camel_case_until("Abc"), StrIndex::new(3, 3));
        assert_eq!(
            _camel_case_until("Abc\u{f6}\u{f6}\u{f6}"),
            StrIndex::new(6, 9)
        );
    }

    #[test]
    fn _camel_case_until_not() {
        assert_eq!(_camel_case_until("abcDef"), StrIndex::new(0, 0));
        assert_eq!(_camel_case_until("aDbc"), StrIndex::new(0, 0));
    }

    #[test]
    fn _camel_case_until_partial() {
        assert_eq!(_camel_case_until("AbcDef_"), StrIndex::new(6, 6));
        assert_eq!(_camel_case_until("CallTypeC"), StrIndex::new(8, 8));
        assert_eq!(_camel_case_until("AbcDD"), StrIndex::new(3, 3));
        assert_eq!(_camel_case_until("Abc\u{f6}\u{f6}DD"), StrIndex::new(5, 7));
    }
