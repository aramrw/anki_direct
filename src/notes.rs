...
impl AnkiClient {
    ...
    /// }
    /// ```
    pub async fn query_note_ids(&self, query: AnkiQuery) -> Result<Vec<isize>, AnkiError> {
...
mod note_tests {
    ...
    async fn find_new_note_ids() {
        let res = ANKICLIENT
            .query_note_ids(AnkiQuery::CardState(CardState::IsNew))
            .await
            .map_err(|e| e.pretty_panic())
            .unwrap();
        assert!(!res.is_empty());
        let res = ANKICLIENT
            .query_note_ids(AnkiQuery::CardState(CardState::IsLearn))
            .await
            .map_err(|e| e.pretty_panic())
            .unwrap();
        assert!(!res.is_empty())
